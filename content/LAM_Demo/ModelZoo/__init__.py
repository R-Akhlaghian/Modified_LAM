...
def get_model(module_name, class_name, package_path=".networks"):
    ...
    with open(f'content/LAM_Demo/ModelZoo/networks/{class_name}.json', 'r') as file:
...
def load_model(module_name, class_name, pretrained_model_name):
    net = get_model(module_name, class_name)
    # state_dict_path = os.path.join(MODEL_DIR, MODEL_LIST[class_name]['Base'])
